...
image_dir = resource_path("cards/lightmode")


# Get a list of all image files in the directory
image_files = [f for f in os.listdir(resource_path("cards/lightmode")) if f.endswith('.png')]
...
for row in range(3):
    for col in range(4):
        ...
        button = tk.Button(root, image=img, width=200, height=150, bd=8, relief="groove", highlightbackground="white")
...
def button_click(button):
    if button["relief"] == "sunken":
        button.config(relief="groove", bd=8, highlightbackground="white")
    else:
        button.config(relief="sunken", bd=8, highlightbackground="black")
    if sum(button["relief"] == "sunken" for button in buttons) == 3:
        ...
        for i in range(4):
            ...
            if counts.count(2) > 0:
                is_set = False
                pygame.mixer.music.load(resource_path("other/wrong.mp3"))
        ...
        if is_set:
            ...
            for button in buttons:
                if button["relief"] == "sunken":
                    ...
                    button.config(image=img, relief="groove", bd=8, highlightbackground="white")
                    button.image = img
...
refresh_button = tk.Button(root, command=refresh_images,
                           ...
                           bd=8, highlightbackground="black")
...
console_box = tk.Text(root,relief="solid",borderwidth = 5, font=("Ubuntu Light", 20), height = 10, width = 28, bg = "grey")
